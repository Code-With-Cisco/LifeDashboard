...
function buildPersonalDataTransaction({commit=false}={}){
 return `BEGIN;
SET LOCAL lock_timeout='5s';
SET LOCAL statement_timeout='90s';
DO $$ BEGIN
 IF to_regclass('public.user_documents') IS NOT NULL THEN
  RAISE EXCEPTION 'Personal documents already exist; inspect migration state instead of reapplying'; END IF;
...
DO $$ DECLARE t record; digest text; BEGIN
 ...
 END LOOP;
END $$;
${read('database/migrations/20260906_005_personal_documents.sql')}
...
DO $$ DECLARE t record; digest text; BEGIN
 ...
 END LOOP;
 IF EXISTS(SELECT 1 FROM public.user_documents) THEN RAISE EXCEPTION 'Document fixtures remain'; END IF;
END $$;
...
if(require.main===module){
 ...
 if(args.length>1||args.some(arg=>arg!=='--commit')){
  process.stderr.write('Usage: node scripts/prepare-personal-data.js [--commit]\n');process.exitCode=1;
 }else process.stdout.write(buildPersonalDataTransaction({commit:args.includes('--commit')}));
}
